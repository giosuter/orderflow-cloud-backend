 ...
 *
 * Focus:
 *  - Verify that GET /api/orders/{id}:
 *      * delegates to OrderService.findById(...)
 *      * returns 200 OK with the expected JSON payload
 *      
 *  This test:    
 *  - increases coverage on the happy path of getOne(...)
 *  - does NOT require a real database or Flyway (OrderService is mocked)
 *  - protects the external REST contract (JSON structure, HTTP status)
 */
...
class OrderControllerGetOneTest {
    ...
    /**
     * Mock the service layer because in a web slice test we want to
     * verify only controller behavior and JSON mapping, not persistence.
     */
    @MockBean
    ...
    void getOne_shouldReturnOrder_whenExists() throws Exception {
        ...
        mockMvc.perform(get("/api/orders/{id}", id)
                ...
                .andExpect(jsonPath("$.code").value("ORD-42"))
